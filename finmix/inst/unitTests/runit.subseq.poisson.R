...
"test.subseq.mcmcoutputbase.poisson" <- function() {
      ...
      checkTrue(all(is.na(mcmcout.sub@S)), "check11")
      checkEquals(nrow(mcmcout.sub@NK), 5)
      checkEquals(ncol(mcmcout.sub@NK), ncol(mcmcout@NK))
      checkTrue(all(mcmcout.sub@NK == mcmcout@NK[1:5,]), "check14")
      ## --- Check for K = 3 --- ##
      set.seed(0)
      setK(model)   <- 3
      prior         <- prior(hier = FALSE)
      ...
      checkEquals(nrow(mcmcout.sub@weight), 5)
      checkEquals(ncol(mcmcout.sub@weight), ncol(mcmcout@weight))
      checkTrue(all(mcmcout@weight[1:5,] == mcmcout.sub@weight), "check21")
      checkEquals(nrow(mcmcout.sub@ST), 5)
      checkEquals(ncol(mcmcout.sub@ST), 1)
      checkTrue(all(mcmcout.sub@ST == mcmcout@ST[1:5]), "check24")
      checkTrue(all(is.na(mcmcout.sub@S)), "check25")
      checkEquals(nrow(mcmcout.sub@NK), 5)
      checkEquals(ncol(mcmcout.sub@NK), ncol(mcmcout@NK))
      checkTrue(all(mcmcout.sub@NK == mcmcout@NK[1:5,]), "check28")
      ## Test exception ##
      sub.index    <- matrix(seq(1:50) < 6, nrow = 50, ncol = model@K)
      checkException(subseq(mcmcout, sub.index), silent = TRUE)
}
...
"test.subseq.mcmcoutputpost.poisson" <- function() {
      ...
      checkEquals(nrow(mcmcout.sub@weight), 5)
      checkEquals(ncol(mcmcout.sub@weight), ncol(mcmcout@weight))
      checkTrue(all(mcmcout@weight[1:5,] == mcmcout.sub@weight), "check27")
      checkEquals(nrow(mcmcout.sub@ST), 5)
      checkEquals(ncol(mcmcout.sub@ST), 1)
      checkTrue(all(mcmcout.sub@ST == mcmcout@ST[1:5]), "check30")
      checkTrue(all(is.na(mcmcout.sub@S)), "check31")
      checkEquals(nrow(mcmcout.sub@NK), 5)
      checkEquals(ncol(mcmcout.sub@NK), ncol(mcmcout@NK))
      checkTrue(all(mcmcout.sub@NK == mcmcout@NK[1:5,]), "check34")
      checkEquals(nrow(mcmcout.sub@post$par$a), 5)
      checkEquals(ncol(mcmcout.sub@post$par$a), ncol(mcmcout@post$par$a))
      checkTrue(all(mcmcout.sub@post$par$a == mcmcout@post$par$a[1:5,]), "check37")
      checkEquals(nrow(mcmcout.sub@post$par$b), 5)
      checkEquals(ncol(mcmcout.sub@post$par$b), ncol(mcmcout@post$par$b))
      checkTrue(all(mcmcout.sub@post$par$b == mcmcout@post$par$b[1:5,]), "check40")
      ## Test exception ##
      sub.index    <- matrix(seq(1:50) < 6, nrow = 50, ncol = model@K)
      checkException(subseq(mcmcout, sub.index), silent = TRUE)
}
