...
setMethod("plot", signature(x = "mcmcoutputpermpost", 
...
setMethod("plotHist", signature(x = "mcmcoutputpermpost", dev = "ANY"), 
	function(x, dev = TRUE, ...) {
	if(x@model@dist == "poisson") {
		K <- x@model@K 
...
setMethod("plot", signature(x = "mcmcoutputpermhierpost", 
	y = "missing"), function(x, y, ...) {
	if (x@model@dist == "poisson") {
		K <- x@model@K
		trace.n <- K * 2 
