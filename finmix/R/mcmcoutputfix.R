           ...
           function( x, dev = TRUE, ... )
           {
               ...
               if ( dist == "poisson" ) {
                  ...
                  .pointproc.Binomial( x, dev )
                   ...
                   .pointproc.Exponential( x, dev )
               } else if ( dist %in% c( "normal", "student" ) ) {
                   .pointproc.Normal( x, dev )
               } else if ( dist %in% c( "normult", "studmult" ) ) {
                   .pointproc.Normult( x, dev )                   
               }
          }
)
...
setMethod( "plotSampRep", signature( x    = "mcmcoutputfix",
           ...
           function( x, dev, ... ) 
           {
               ...
               if (dist %in% c( "poisson", "exponential" ) ) {
                   ...
                   .samprep.Normal( x, dev )
               } else if ( dist == "student" ) {
                   .samprep.Student( x, dev )
               } else if ( dist == "normult" ) {
                   .samprep.Normult( x, dev )
               } else if ( dist == "studmult" ) {
                   .samprep.Studmult( x, dev )
               }
           }
)
...
".traces.Normal" <- function( x, dev ) 
{
    ...
    for ( k in 1:K ) {
        ...
        axis( 2, las = 2, cex.axis = .7 )              
        mtext( side = 2, las = 2, bquote( sigma[k = .( k )]^2),
               cex = .6, line = 3 )        
    }
    axis( 1 ) 
    mtext( side = 1, "Iterations", cex = .7, line = 3 )
...
".hist.Normal" <- function( x, dev )
{
    ...
    if ( K == 1 ) {
        if ( .check.grDevice() && dev ) {
            dev.new( title = "Histogram Mean" )
        }
        .symmetric.Hist( mu, list( bquote( mu ) ) )
        if ( .check.grDevice() && dev ) {
            dev.new( title = "Histogram Variance" )
        }
        ...
        if ( .check.grDevice() && dev ) {
            dev.new( title = "Histograms Mean" )
        }
        .symmetric.Hist( mu, mu.lab.names )
        if ( .check.grDevice() && dev ) {
            dev.new( title = "Histograms Variance" )
        }
...
".hist.Normult"  <- function( x, dev ) 
{
    ...
    for ( rr in 1:r ) {
        if ( K == 1 ) {
            ...
            .symmetric.Hist( sigma[, index[rr],], list( bquote( sigma ) ) )           
        } else {
            mu.lab.names    <- vector( "list", K )
            sigma.lab.names <- vector( "list", K ) 
            for ( k in 1:K ) {
            ...
            .symmetric.Hist( sigma[, index[rr],], sigma.lab.names )
        }        
    }
}
...
".dens.Normal" <- function( x, dev )
{
    ...
    if ( K == 1 ) {
        if ( .check.grDevice() && dev ) {
            dev.new( title = "Density Mean" )
        }
        .symmetric.Dens( mu, list( bquote( mu ) ) )
        if ( .check.grDevice() && dev ) {
            dev.new( title = "Density Variance" )
        }
        ...
        if ( .check.grDevice() && dev ) {
            dev.new( title = "Densities Mean" )
        }
        .symmetric.Dens( mu, mu.lab.names )
        if ( .check.grDevice() && dev ) {
            dev.new( title = "Densities Variance" )
        }
...
".dens.Normult"  <- function( x, dev ) 
{
    ...
    for ( rr in 1:r ) {
        if ( K == 1 ) {
            ...
            .symmetric.Dens( sigma[, index[rr],], list( bquote( sigma ) ) )           
        } else {
            mu.lab.names    <- vector( "list", K )
            sigma.lab.names <- vector( "list", K ) 
            for ( k in 1:K ) {
            ...
            .symmetric.Dens( sigma[, index[rr],], sigma.lab.names )
        }        
    }
    if ( .check.grDevice() && dev ) {
...
".dens.Studmult"  <- function( x, dev ) 
{
    ...
    for ( rr in 1:r ) {
        if ( K == 1 ) {
            ...
            .symmetric.Dens( sigma[, index[rr],], list( bquote( sigma^2 ) ) )
...
".pointproc.Exponential" <- function( x, dev )
{
    K   <- x@model@K
    M   <- x@M
    if ( .check.grDevice() && dev ) {
...
".pointproc.Normal" <- function( x, dev ) 
{
    K   <- x@model@K
    M   <- x@M
    if ( .check.grDevice() && dev ) {
...
".pointproc.Normult"    <- function( x, dev ) 
{
    K       <- x@model@K
    M       <- x@M
    r       <- x@model@r    
    ...
    comb    <- choose( r, 2 )
    par( mfrow = c( choose( r, 2 ), 1 ), mar = c( 3, 3, 1, 1 ), 
         oma = c( 4, 5, 4, 4 ) )
    m               <- 1
    for ( i in 1:( r - 1 ) ) {
        for ( j in ( i + 1 ):r ) {
...
".samprep.Poisson" <- function( x, dev )
{
    ...
    lambda  <- x@par$lambda
    if ( .check.grDevice() && dev ) {
        dev.new( title = paste( "Sampling Representation K = ", K, sep = "" ) )
    }
...
".samprep.Student"   <- function( x, dev ) 
{
    K   <- x@model@K
    if ( K == 1 ) {
        warning( paste( "Sampling representation is only ",
...
".samprep.Normult"   <- function( x, dev ) 
{
    K   <- x@model@K
    if ( K == 1 ) {
        warning( paste( "Sampling representation is only ",
    ...
    for ( rr in 1:r ) {
        if ( .check.grDevice() && dev ) {
            dev.new( title = paste( "Sampling Representations Feature ", rr, 
                                    ", K = ", K, sep = "" ) )
        }
        mur     <- mu[, rr, ]
        mur     <- mur[seq( 1, n ), ]
...
".samprep.Studmult"   <- function( x, dev ) 
{
    K   <- x@model@K
    if ( K == 1 ) {
        warning( paste( "Sampling representation is only ",
    ...
    if ( .check.grDevice() && dev ) {
        dev.new( title = paste( "Sampling Representations Degrees of Freedom K = ", 
                                K, sep = "" ) )
    }
    degf        <- degf[seq( 1, n ), ]
    degf        <- matrix( degf[, comb], nrow = n * n.perm, ncol = 2 )
...
".postdens.Normal"  <- function( x, dev ) 
{
    K   <- x@model@K
    if ( K != 2 ) {
        warning( paste( "A plot of the posterior density is ",
...
".postdens.Student"  <- function( x, dev ) 
{
    K   <- x@model@K
    if ( K != 2 ) {
        warning( paste( "A plot of the posterior density is ",
...
".postdens.Normult"  <- function( x, dev ) 
{
    K   <- x@model@K
    if ( K != 2 ) {
        warning( paste( "A plot of the posterior density is ",
...
".postdens.Studmult"  <- function( x, dev ) 
{
    K   <- x@model@K
    if ( K != 2 ) {
        warning( paste( "A plot of the posterior density is ",
