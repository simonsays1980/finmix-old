                                       ...
                                       prior.obj, mcmc.obj, n.args )
{
    ...
    tt <- tryCatch( .check.fdata.model.Mcmcstart( fdata.obj, model.obj ),
    ...
    ## Check if model is a Student-t mixture and if in this case
    ## starting degrees of freedom have been provided. 
    if ( model.obj@dist %in% c( "student", "studmult" ) ) {
        if ( !hasDf( model.obj, verbose = FALSE ) ) {
            hasDf( model.obj, verbose = TRUE )             
        } else {
            if ( !is.null( tt ) ) {
                warning( tt, call. = FALSE )                        
            }
        }
    }
    return( mcmc.obj )
        ...
        if ( !prior.obj@hier ) {
            ...
            if ( !mcmc.obj@storepost ) {
                ...
                mcmcout     <- .mcmcoutputfixpost( M = M, burnin = burnin, 
                                                   ...
                                                   prior = prior.obj )
                .Call( "mcmc_normult_cc", fdata.obj, model.obj, prior.obj, 
                       mcmc.obj, mcmcout, PCKAGE = "finmix" )
                mcmcout@par$storeinv <- NULL
                return( mcmcout ) 
            }
            ## end no hier
